...
/*   Updated: 2023/04/18 15:17:52 by mich             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redirection(t_shell *shell)
{
	...
	shell->redirection.count_redirection = ft_sarsize(shell->lst.redirection) - 1;
	if (shell->redirection.count_redirection >= 1)
	{
		clean_parse(shell);
		exp_red(shell);
		if (shell->redirection_id == 1)
		{
			shell->lst.delete_str = (char **)malloc(sizeof(char *) \
				* (shell->redirection.count_redirection + 1));
			while (shell->redirection.count_redirection > 0)
			{
				shell->lst.file = \
					ft_split(shell->lst.redirection[shell->redirection.count_redirection], ' ');
				red_out(shell->redirection.count_redirection, shell, shell->redirection.count_delete_str, shell->redirection.j);
				shell->redirection.count_redirection--;
				shell->redirection.count_delete_str++;
				ft_sarfree(shell->lst.file, ft_sarsize(shell->lst.file));
			}
			shell->lst.delete_str[shell->redirection.count_delete_str] = NULL;
		}
		...
		else if (shell->redirection_id == 3)
		{
			...
			while (shell->redirection.count_redirection > 0)
			{
				shell->lst.file = \
					ft_split(shell->lst.redirection[shell->redirection.count_redirection], ' ');
				append(shell, shell->redirection.j, shell->redirection.count_redirection, shell->redirection.count_delete_str);
				shell->redirection.count_redirection--;
				shell->redirection.count_delete_str++;
				ft_sarfree(shell->lst.file, ft_sarsize(shell->lst.file));
			}
